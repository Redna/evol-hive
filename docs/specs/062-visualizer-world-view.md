...
- [ ] **AC-1** (R1): for the coffee-shop fixture, `layoutWorld` returns a 2×2 placement in which **≥ 4 of the 5** connections are edge-adjacent, and the single non-adjacent connection becomes a `'corridor'` whose polyline does not intersect any room rect.
...
- [ ] **AC-5** (R4): `layoutWorld` output is deep-equal for two different `Skin` implementations (layout is skin-independent); the default skin satisfies the `Skin` interface.
...
- [ ] **AC-9** (R8): the spec-042 invariant test is updated so drawing is permitted only under `src/renderer/**`; all other visualizer tests pass unmodified; `pnpm -r test && pnpm typecheck && pnpm lint` pass.
...
- **Measured evidence** (prototype, iPhone-class 390×844): 2×2 topology-scored placement, 4/5 connections edge-adjacent, the diagonal one routed through the gutter; no room outside the viewport at 1280×720 / 390×844 / 360×640; object text clipped to its chip; tap-select verified. Before the fix the same code produced a 1-column strip on portrait and desktop overflow.
