...
- **Amended by [066](066-visualizer-live-observation-defects.md) (R3, leg 1 — merged).** This spec's R3 said that clearing the selection returns "the fit-all view at the previous scale". A live run showed the release landed at the **follow** zoom (1.6×) with zeroed offsets — a magnified view anchored on the world origin, reported as "when I release the focus I get dragged to nowhere" (spec 066, D2). The release is now **history-independent fit-all** (`cameraFor(null, …)` ignores `previous`; the layout is already centred at scale 1), and the `scale` transition **glides** via the same time-based smoother as the offsets instead of snapping. The R3 wording is not rewritten here — it is amended in place so the ledger carries the correction.
- **AC-9 (the phone walk) has now been exercised** — spec 066's Context records what it found. The sustained real-LLM run over the phone path surfaced **four live defects** the suite could not catch (three on code paths no test executed): co-located agents drawn on top of each other while tapping one selected another (D1); releasing follow parked the camera at 1.6× on the origin (D2); motion aliased into teleporting because the glide rate ignored the ~100 ms snapshot interval (D3); and two control-bar controls misreported state — the scene selector showed `minimal` while coffee-shop ran, and the Fog button looked off while the view started fogged, which invalidated one of the diagnostics (D4, closed by removal of the lying controls). All four are fixed under spec 066; the D1/D2/D3 legs were live-validated on a rebuilt `dist` (see the spec-066 implementation notes).
- **AC-9 finding left to a separate change (not spec 066).** On iOS the home-screen install is degraded: `apple-touch-icon` points at an **SVG** (iOS needs a PNG for the home-screen icon) and `apple-mobile-web-app-capable` is absent. Spec 066 deliberately scopes to defects that break *use*, not polish, and leaves the iOS icon/capability gap to the issue that already tracks it. Recorded here so the AC-9 closure is honest about what the walk found and what was not fixed.
