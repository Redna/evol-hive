...
- **Empty-args root cause** (`af0fa60`, 2026-09-07): the empty `formulate_plan` args are an **intermittent backend behavior on tool-loop turn 2** — they strike only on the formulate_plan call *after* a tool result turn (e.g. `observe_agent`). Exposure asymmetry explained: the apprentice's directives make him observe before planning (~100% exposure → 508 failed plans), while the gardener plans directly (~5% → 81). Static and synthetic multi-turn repros do NOT reproduce it; diagnostics: `[llm-raw]` dumps the full failing payload (first occurrence per agent) to `/tmp/empty-args-<agentId>.json`; `scripts/repro-empty-args.mts` (5 payload variants) and `scripts/replay-payload.mts` (replay + bisect) reproduce the surrounding behavior.
