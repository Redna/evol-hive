...
10. **Contextual forcing in Perception/Action-choice phase** — `PerceptionBuilderImpl.build()` must accept an optional `hasPlan: boolean` and `forcingEnabled: boolean`. When `hasPlan === false` and `forcingEnabled === true`, the `GUARDRAIL_FORCING_DIRECTIVE` is appended to the system prompt. When `hasPlan === false` and `affordanceMasking === true`, the `availableAffordances` in the `LLMContextPayload` are set to `[]` and the `tools` array is reduced to only cognitive tool definitions (no `chooseActionTool`).
...
- [ ] AC-15: When the agent has no plan and affordance masking is enabled, the `PerceptionResult.prunedAffordances` is an empty array. *(Maps to Req 8)*
- [ ] AC-16: When the agent has a plan and affordance masking is enabled, the `PerceptionResult.prunedAffordances` is unchanged from classifier output. *(Maps to Req 8)*
