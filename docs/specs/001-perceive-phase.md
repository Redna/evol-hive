...
1. **Room state retrieval** — The engine must expose a method to retrieve all `SmartObject` instances located in the agent's current room (`AgentInternalState.location`), returning only `id`, `name`, and `type` (no deep `state`).

2. **Affordance collection** — The engine must expose a method to aggregate all `Affordance[]` across every `SmartObject` in a given room, returning a flat list suitable for classifier input.

3. **Primary drive determination** — The engine must compute the agent's primary drive as the drive with the **lowest** value (0 = most urgent, per §3), and produce a semantic label string in the format `"low {drive}, need to restore {drive}"` for the classifier query.

4. **Spatial debounce — room threshold** — The engine must trigger a new perception tick when the agent crosses a room boundary (i.e., `AgentInternalState.location` changes since the last tick).

5. **Spatial debounce — idle timer** — The engine must trigger a new perception tick when the agent has been idle (no movement, no action execution) for longer than `EngineConfig.spatialDebounceSeconds` (default: 30s).

6. **Perception tick recording** — After each perception tick fires, the engine must update `AgentInternalState.lastPerceptionTick` to the current simulation time.

7. **Debounce skip** — If neither the room-threshold nor idle-timer condition is met, the engine must **not** trigger a perception tick and must return a sentinel indicating "no perception needed."

### Cognition Layer (`@evol-hive/cognition`)

8. **Passive perception assembly** — The cognition layer must build a `PassivePerception` object containing: the agent's current `roomId`, an array of `{ objectId, name, type }` for each smart object in the room, a snapshot of the agent's current `drives`, any pending `systemFeedback` string from a failed action, and (optionally) `associativeMemories` from Track 1.

9. **System 0 classifier pruning** — The cognition layer must invoke `AffordanceClassifier.prune()` with the primary drive label (from Req 3) and the full room affordance list (from Req 2), returning only the top-K affordances whose cosine similarity meets or exceeds the configured threshold (default K=5, threshold=0.3).
...
- [ ] **AC-5**: When `AgentInternalState.location` changes from `"kitchen"` to `"lounge"`, a perception tick is triggered. *(Req 4)*
- [ ] **AC-6**: When `AgentInternalState.location` does **not** change between ticks, no room-threshold perception tick is triggered. *(Req 4)*
- [ ] **AC-7**: When the agent has been idle for `spatialDebounceSeconds + 1` seconds (simulation time), a perception tick is triggered. *(Req 5)*
- [ ] **AC-8**: When the agent has been idle for less than `spatialDebounceSeconds`, no idle-timer perception tick is triggered. *(Req 5)*
- [ ] **AC-9**: After a perception tick fires, `AgentInternalState.lastPerceptionTick` equals the current simulation time. *(Req 6)*
- [ ] **AC-10**: When neither debounce condition is met, `shouldPerceive()` returns `false` and no perception tick fires. *(Req 7)*
...
- [ ] **AC-18**: `PerceptionBuilder.build(perceptionResult)` returns an `LLMContextPayload` with `availableAffordances` set to the pruned list, `perceptionContext` containing the room name and object names, and `responseSchema` set to the structured output schema. *(Req 11)*
...
- [ ] **AC-21**: When `systemFeedback` is present (from a prior failed action), it is included in the `PassivePerception.systemFeedback` field. *(Req 8)*
...
- **Configurable thresholds**: `CLASSIFIER_TOP_K`, `CLASSIFIER_SIMILARITY_THRESHOLD`, and `spatialDebounceSeconds` must be config/env-driven, not hardcoded constants. *(§5 Configuration table)*
