...
export function assembleGameLoop(
  ...
  // (5) AutoSaveSystem — last priority, only when enabled and persistence is available (spec 017, Req 18).
...
export function loadScene(core: EngineCore, scene: SceneDefinition): void {
  // Rooms — use restoreRooms so the SceneManagerImpl reference is preserved
  // (the EnginePersistenceImpl, if present, holds the same reference — spec 017).
  const roomMap = new Map<string, Room>();
  for (const room of scene.rooms) {
    roomMap.set(room.id, room);
  }
  core.sceneManager.restoreRooms(roomMap);
