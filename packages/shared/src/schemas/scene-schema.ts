...
export const sceneDefinitionSchema = {
  ...
  description:
    'Declarative scene definition for evol-hive (spec 022). Maps 1:1 to the SceneDefinition interface in packages/shared/src/types/world.ts.',
  type: 'object',
  required: ['id', 'name', 'rooms', 'objects', 'agents'],
  ...
  $defs: {
    Room: {
      type: 'object',
      required: ['id', 'name', 'description', 'connections', 'objectIds'],
      additionalProperties: false,
      properties: {
        id: {
          type: 'string',
    ...
    SmartObject: {
      type: 'object',
      required: ['id', 'name', 'type', 'state', 'affordances', 'roomId'],
      additionalProperties: false,
      properties: {
        id: {
          type: 'string',
        ...
        type: {
          type: 'string',
          description:
            'Object type for affordance grouping (e.g., appliance, fixture, furniture, doorway, nature).',
        },
        state: {
          type: 'object',
    ...
    Affordance: {
      type: 'object',
      required: ['id', 'label', 'engineEffect', 'preconditions', 'effects'],
      additionalProperties: false,
      properties: {
        id: {
          type: 'string',
        ...
        stepGroup: {
          type: 'string',
          description:
            'Semantic group name linking related affordances into a multi-step sequence (spec 018).',
        },
        stepOrder: {
          type: 'number',
          description:
            "1-based ordinal indicating the step's position within its stepGroup (spec 018).",
        },
        conditions: {
          type: 'array',
          description:
            'Structured conditions evaluated at perception time to determine availability (spec 018).',
          items: {
    ...
    ObjectStateRule: {
      type: 'object',
      required: ['field', 'operation', 'rate', 'interval'],
      additionalProperties: false,
      properties: {
        field: {
          type: 'string',
    ...
    CompoundAction: {
      ...
      properties: {
        ...
        steps: {
          ...
          items: {
            type: 'object',
            required: ['affordanceId', 'description'],
            additionalProperties: false,
            properties: {
              affordanceId: {
                type: 'string',
    ...
    ObjectDependency: {
      type: 'object',
      required: ['affordanceId', 'requiresObjectId', 'requiresAffordance', 'description'],
      additionalProperties: false,
      properties: {
        affordanceId: {
          type: 'string',
    ...
    AgentProfile: {
      type: 'object',
      required: ['id', 'name', 'description', 'traits', 'initialDrives'],
      additionalProperties: false,
      properties: {
        id: {
          type: 'string',
        ...
        startRoomId: {
          type: 'string',
          description:
            'Optional room ID where the agent spawns. When absent, the first room is used.',
        },
      },
    },
  },
