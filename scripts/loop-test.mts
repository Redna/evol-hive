...
const TOOLS = [
  { type: 'function', function: { name: 'observe_agent', description: 'Observe another agent', parameters: { type: 'object', properties: { targetAgentId: { type: 'string' } }, required: ['targetAgentId'] } } },
  {
    type: 'function',
    function: {
      name: 'formulate_plan',
      description: "Create a plan to satisfy the agent's drives. EVERY step MUST set targetAffordance to one of the enum values (use 'wait' when nothing is relevant).",
      parameters: {
        type: 'object',
        properties: {
          ...
          steps: {
            ...
            items: {
              ...
              properties: {
                ...
                targetAffordance: { type: 'string', description: "The affordance ID to execute for this step. MUST be one of the enum values. Use 'wait' when no affordance is relevant.", enum: ['plant_seeds', 'harvest', 'eat', 'observe', 'wait'] },
              },
...
async function turn(): Promise<{ tool: string; args: string }> {
  ...
  const d = (await res.json()) as { choices: { message: { tool_calls?: { function: { name: string; arguments: string } }[] } }[] };
...
messages.push({ role: 'assistant', content: null, tool_calls: [{ id: 'c1', type: 'function', function: { name: t1.tool, arguments: '{"targetAgentId":"gardener-1"}' } }] });
messages.push({ role: 'tool', content: JSON.stringify({ agentId: 'gardener-1', name: 'Maren Holt', drives: { energy: 99, hunger: 98, social: 70, comfort: 90, curiosity: 95 }, currentPlan: { description: 'Farm', steps: [{ description: 'Plant seeds', targetAffordance: 'plant_seeds' }] }, location: 'garden' }), tool_call_id: 'c1' });

// Turn 2: this is where the live run sees args:{} — sample it 5 times.
for (let i = 0; i < 5; i++) {
  messages.pop(); messages.pop(); // re-push fresh each time
  messages.push({ role: 'assistant', content: null, tool_calls: [{ id: `c${i}t2`, type: 'function', function: { name: t1.tool, arguments: '{"targetAgentId":"gardener-1"}' } }] });
  messages.push({ role: 'tool', content: JSON.stringify({ agentId: 'gardener-1', name: 'Maren Holt', drives: { energy: 99, hunger: 98 }, location: 'garden' }), tool_call_id: `c${i}t2` });
  const t2 = await turn();
  const flag = t2.tool === 'formulate_plan' && (t2.args === '{}' || t2.args === '') ? ' *** EMPTY ARGS ***' : '';
