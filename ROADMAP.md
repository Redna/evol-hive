...
**41 specs, 160+ PRs — all merged. 2,187+ tests passing.**
...
> The cognitive core is complete. Current focus: **behavioral verification of the spatial layer** (spec 039 shipped: targetArea intents, cell fog, social fog-lifting, determinism), the System 1 label pipeline (wait-only ignore refinement + decay-noise threshold + exploration ε — dream v4 trained on 7.4K samples, headVersion 3→4), and the label pipeline, now causally complete (specs 040/041: idle-tick suppression + applied-driveChanges labeling — the head trains on what cycles DID, not on physics). ~~Drive-economy equilibrium~~ ✅ (#139). ~~First conversations~~ ✅ (2026-09-08 — `talk_to` exchanges observed live: herb offers, work offers, trust deltas).
